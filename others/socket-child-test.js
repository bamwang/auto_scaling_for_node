...
function childProcessManager(){

	var RunningList =function(){};
	var StopList =function(){};
	var _running = new RunningList();
	var _stop = new StopList();
	...
	var _removeWorkerFromById = function(CPId, list){
		var id = CPId;
		delete list[id];
		return [_stop,_running];
	}
	...
	this.getIdleWorker = function(){
		var ids=Object.keys(_stop);
		if(ids.length>0){
			var id = ids[0];
			var worker = _getWorkerFrom(id, _stop);
			_removeWorkerFrom(worker, _stop);
			_addWorkerTo(worker, _running);
			return worker;
		}
		return undefined;
	}
	this.returnIdleWorker = function(worker){
		//todo: add validation of worker
		_removeWorkerFrom(worker, _running);
		_addWorkerTo(worker, _stop);
		return [_stop,_running];
	}
	this.addNewWorker = function(worker){
		//todo: add validation of worker
		return _addWorkerTo(worker, _stop);
	}
	this.getList = function(){
		return [_stop,_running];
	}
	this.getAliveWorker = function(){
		var idInIdle=Object.keys(_stop);
		var idInBusy=Object.keys(_running);
		return idInBusy.length+idInIdle.length;
	}
}

var cpm = new childProcessManager();
var i = 0
for (; i <= MIN_WORKER; i++) {
 	var child=cp.fork(__dirname + '/socket-test-2.js');
 	child.kill('SIGCHLD');
 	cpm.addNewWorker(child);
 }; 
